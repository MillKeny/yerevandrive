...
def selectTrack(gamepath, track):
    tracks_source = os.path.join(os.path.dirname(__file__), 'tracks')
    tracks_dest = os.path.join(gamepath, 'Tracks')
    
    source_file = os.path.join(tracks_source, track)
...
def restoreTracks(gamepath):
    ...
    if not os.path.exists(tracks_source):
        raise FileNotFoundError(f"Source tracks folder not found: {tracks_source}")
